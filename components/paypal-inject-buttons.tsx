    ...
    const injectPayPalButtons = () => {
      if (typeof window !== "undefined" && (window as any).cartPaypal) {
        ...
        products.forEach((product) => {
          const paypalButtonId =
            storeConfig.paypal.buttonIds[
              product.id as keyof typeof storeConfig.paypal.buttonIds
            ];
          if (paypalButtonId) {
            const container = document.getElementById(
              `paypal-add-to-cart-${product.id}`
            );
            if (container) {
              container.innerHTML = `<paypal-add-to-cart-button data-id="${paypalButtonId}"></paypal-add-to-cart-button>`;

              const addToCart = (window as any).cartPaypal?.AddToCart;
              if (addToCart) {
                addToCart({ id: paypalButtonId });
                console.log(
                  `[PayPal] Add to Cart button injected for product ${product.id}`
                );
              }
            }
          }
